...
let r1prog_to_prog prog =
  match prog with
  | R1.AProgram (i, exp) ->
    AProgram (("locals", []), ["start", r1_to_tail exp]) 
...
let rec uncover_locals_in_all_labels ls accum =
  match ls with
  | [] -> List.rev accum
  | (_, t) :: ls -> uncover_locals_in_all_labels ls (uncover_locals_in_tail t accum)
                         
let uncover_locals (AProgram (i, l)) =
  let locals_ls = uncover_locals_in_all_labels l [] in
